...
INITIAL_RTO, MIN_RTO, MAX_RTO = 0.03, 0.01, 0.5
BACKOFF = 1.1
FAST_DUP_THRESH = 2
FAST_RETX_BATCH = 5
...
class P1Server:
    def __init__(self, ip, port, sws_bytes):
        ...
        with open("data.txt", "rb") as f: self.data = f.read()
        ...
        print(f"[Server] Listening {ip}:{port} | SWS={sws_bytes}")
    ...
    def update_rto(self, rtt):
        ...
        adj = 0.7 * self.min_rtt + 0.3 * rtt
        if self.srtt == 0:
            self.srtt, self.rttvar = adj, adj / 2
        else:
            self.rttvar = (1 - BETA) * self.rttvar + BETA * abs(self.srtt - adj)
            self.srtt = (1 - ALPHA) * self.srtt + ALPHA * adj
    ...
    def fill_pipe(self):
        ...
        limit = min(16 * self.sws, self.size)
        while in_flight < limit:
            seq, d, f = self.next_chunk()
            if seq is None: break
            self.send_pkt(seq, d, f)
            if f & EOF_FLAG: break
            in_flight = self.next_seq - self.base
    ...
    def handle_ack(self, pkt):
        ...
        if ack == self.base:
            ...
            if self.dup_acks >= FAST_DUP_THRESH:
                for s in list(self.sent.keys())[:FAST_RETX_BATCH]:
                    self.sock.sendto(self.sent[s][0], self.client)
                self.rto = min(self.rto * BACKOFF, MAX_RTO)
            return "CONT"
        if ack > self.base:
            self.dup_acks = 0
            ...
            if acked:
                newest = max(acked)
                pktx, st, rc = self.sent[newest]
                if rc == 0:
                    self.update_rto(now - st)
            for s in acked: self.sent.pop(s, None)
            self.base = ack
            if flags & EOF_FLAG and ack > self.size: return "DONE"
        if sack_e > sack_s:
    ...
    def check_timeouts(self):
        ...
        for s in expired[:FAST_RETX_BATCH]:
            pkt, _, rc = self.sent[s]
            self.sent[s] = (pkt, now, rc + 1)
            self.sock.sendto(pkt, self.client)
        if expired: self.rto = min(self.rto * BACKOFF, MAX_RTO)

    def run(self):
        print("Waiting for client...")
        readable, _, _ = select.select([self.sock], [], [], 15)
        if not readable:
            print("No client request."); return
        ...
        while True:
            ...
            if rlist:
                ...
                if self.handle_ack(pkt) == "DONE": break
            self.check_timeouts()
            self.fill_pipe()
            if self.eof_sent and not self.sent: break
        dur = time.time() - self.start_time
        print(f"✅ Transfer done: {self.size} bytes in {dur:.2f}s")
        self.sock.close()

def main():
    if len(sys.argv) != 4:
        print("Usage: python3 p1_server.py <IP> <PORT> <SWS>"); sys.exit(1)
    P1Server(sys.argv[1], sys.argv[2], int(sys.argv[3])).run()
